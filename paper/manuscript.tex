...
\newcommand{\repoOwn}{loop-class-closure-repro}
...
\item a strict-EXACT closure of every Standard-Model fermion mass:
      ...
      sample gives FACTOR2 $8.4\%$ at $0.004428$, both inside
      the $\le 10\%$ FACTOR2 band, with the closure regime-dependent
...
script bundled at
\pyown{src/prospective/compute_prospective_residuals.py}{compute_prospective_residuals},
...
\pyown{data/prospective_cluster_residuals.json}{prospective_cluster_residuals.json} feeds
...
(reproducer
\pyown{src/prospective/extend_c5_d1embedded_to_higher_N.py}{extend_c5_d1embedded_to_higher_N}).
...
\begin{tabular}{@{}l p{0.36\textwidth} p{0.46\textwidth}@{}}
\toprule
...
PROSP-01 & gravitational-coupling scaling on
          ...
          & bare-$\Phi$ Newtonian-pass $1.000$ on
            ...
            N-range), so the bare-$\Phi$ Newtonian-asymptote
            mode holds uniformly across the full canonical
            ladder and the
...
Fisher combination is not yet defined ($k\!=\!0$ in-band
...
The bundled \pyown{src/inter_annotator_protocol_template.py}{inter_annotator_protocol_template}
runs an inter-annotator consistency audit on the
\emph{operational} workflow: it loads the bundled tuple
assignments (\pyown{data/observable_registry.json}{observable_registry.json}) as the
...
annotator
\pyown{data/inter_annotator_csv/A2_rule_based.csv}{A2_rule_based.csv} was
...
\pyown{outputs/inter_annotator_protocol.json}{inter_annotator_protocol.json} reports
...
identified by the full tuple. The ``is a function'' part is
trivial for any distinct-key lookup table; the substantive
content is the \emph{faithfulness} claim --- the ten library
entries were defined \emph{physics-first} (each is a distinct
loop topology: Yukawa-Damping, PMNS-Self-Energy, $\dots$), and
the theorem asserts that the five-component tuple representation
is rich enough to \emph{separate} all ten physics-motivated
classes injectively. This could have failed and is a near
thing: the cells $(1,0,0)$ and $(4,0,0)$ are each
double-occupancy (L1/L2 and L3/L4 respectively), separated
\emph{only} by a single boolean flag ($w$ or $r$). The theorem
is therefore an injectivity/faithfulness result on the
enumerated library, not a completeness claim about all possible
loop topologies (see the scope statement below). The discriminating power of the
...
\pyown{src/verify_tuple_exhaustion.py}{verify_tuple_exhaustion} of the present
repository, with output
\pyown{outputs/verify_tuple_exhaustion.json}{verify_tuple_exhaustion.json}. The audit scans
...
(\pyown{src/verify_tuple_exhaustion.py}{verify_tuple_exhaustion}; bundled output
\pyown{outputs/verify_tuple_exhaustion.json}{verify_tuple_exhaustion.json}) confirms that the
...
\pyown{data/reclassification_cases.json}{reclassification_cases.json}.
...
\begin{table}[!htbp]
\centering\footnotesize
\input{tables/tab_reclassification_audit.tex}
...
\pyown{data/lemma9_federer_extension.json}{lemma9_federer_extension.json}.  It is intentionally
...
$V_{ub} = \gamma/N_{\rm gen}^{3} = 1/270 = 0.003704$ EXACT $0.37\%$ ($z\!=\!+0.12\sigma$ vs PDG~2024 exclusive determination $0.00370\!\pm\!0.00012$, the cleanest determination from $B\!\to\!\pi\ell\nu$ form-factor lattice QCD); the framework supports the exclusive over the inclusive value $0.00420\!\pm\!0.00013$, the $|V_{ub}|^{\rm incl}/|V_{ub}|^{\rm excl}\!=\!1.135$ tension being the well-known $B\!\to\!X_{u}\ell\nu$ shape-function vs lattice-form-factor schism, with the framework's $\gamma/N_{\rm gen}^{3}$ matching the lattice-QCD-extracted exclusive value as expected from a structural identity using only System-$\mathcal{R}$ rationals; supersedes legacy $\alpha_{\rm EM}/2$, $V_{ts}\!=\!\alpha_{\xi}/(2(2 d+N_{\rm gen}))\!=\!9/220$ EXACT $0.22\%$ (degenerate with $V_{cb}$ as expected from Wolfenstein), $m_{b}/m_{\tau}\!=\!2\!+\!\gamma\!+\!s_{\rm face}\!=\!47/20\!=\!2.350$ EXACT $0.11\%$ (vs PDG $4.18/1.7768\!=\!2.353$), $\Delta m_{31}^{2}\!=\!(m_{3}^{(\nu)})^{2}\!=\![m_{e}\,\gamma^{d+3}(1\!-\!\gamma^{2}(d{+}N_{\rm gen})/4)]^{2}\!=\!0.002523$~eV$^{2}$ EXACT $0.32\%$ vs NuFIT~6.1 $0.002515\!\pm\!0.000028$~eV$^{2}$ (NH-min reading $m_{1}\!\to\!0$; tree $(d{+}1)^{2}\gamma^{4}\!=\!25/10^{4}\!=\!0.0025$~eV$^{2}$ at $0.6\%$ PRECISE is the leading approximation to $m_{3}^{2}$, with the universal $\gamma^{2}(d{+}N_{\rm gen})/4$-leakage dressing applied at the $m_{3}$-mass level promoting to strict-EXACT; companion paper P4B Eq.~p4b\_m3\_nu\_dressed),
$D/H\!=\!(d\!+\!1)\,\gamma^{5}/2\!\cdot\!(1\!+\!\gamma^{2}(d{+}N_{\rm gen})/4)\!=\!2.544\!\times\!10^{-5}$ EXACT $0.13\%$ vs PDG $2.547\!\pm\!0.025\!\times\!10^{-5}$ ($z\!=\!+0.12$; tree $(d{+}1)\gamma^{5}/2\!=\!2.5\!\times\!10^{-5}$ at $1.85\%$; same universal $\gamma^{2}(d{+}N_{\rm gen})$-leakage coefficient $1/d$ as the baryon-asymmetry $\eta_{B}$ and the heavy-light quark mass ratio $m_{b}/m_{s}$, identifying the BBN-correlation family as a single $\gamma^{2}$-order carrier-defect correction across three primordial-cosmology observables),
...
$y_{u}\!=\!(d\!+\!1)\,\gamma^{5}/4$ giving the tree prediction $m_{u}^{\rm tree}\!=\!2.17$~MeV (within PDG $\pm 0.49$~MeV); the parameter-free three-loop carrier-defect dressing $(1\!-\!(d\!+\!1)\gamma^{3})\!=\!(1\!-\!5/1000)$ promotes the closure to $m_{u}\!=\!2.159$~MeV strict-EXACT $0.04\%$ vs PDG~2024 central $2.16$~MeV, completing the up-sector parallel to the charged-lepton three-loop compounds $(1\!+\!2\gamma^{3})$ for $y_{\tau}$ and $(1\!+\!11\gamma^{3})$ for $y_{\mu}$ (running $\{N_{\rm chir},d\!+\!1,2 d\!+\!N_{\rm gen}\}$ over the same three-loop carrier-defect integer-coefficient sequence; minus sign for $y_{u}$ reflects up-type isospin anti-symmetrisation),
$y_{d}\!=\!\alpha_{\rm EM}\,N_{\rm gen}\,\gamma^{2}/(2 d)\!=\!3\alpha_{\rm EM}/800$ giving the tree prediction $m_{d}^{\rm tree}\!=\!4.76$~MeV; the parameter-free two-chirality Pure-Self-Energy dressing $(1\!-\!N_{\rm chir}\gamma^{2})\!=\!(1\!-\!2\gamma^{2})$ promotes the closure to $m_{d}\!=\!4.665$~MeV strict-EXACT $0.111\%$ vs PDG $4.67\!\pm\!0.07$~MeV (Lemma-$3$ Pure-Self-Energy with two left/right Dirac chiralities),
$y_{s}\!=\!\alpha_{\rm EM}\,\gamma\,N_{\rm gen}\,s_{\rm face}\!=\!9\alpha_{\rm EM}/120$ giving the tree prediction $m_{s}^{\rm tree}\!=\!95.2$~MeV; the \emph{same} parameter-free $(1\!-\!N_{\rm chir}\gamma^{2})$ Pure-Self-Energy dressing promotes the closure to $m_{s}\!=\!93.30$~MeV strict-EXACT $0.111\%$ vs PDG $93.4\!\pm\!8.6$~MeV (down-quark and strange-quark close at the identical Lemma-$3$ Pure-Self-Energy correction, preserving the bare-EXACT ratio $m_{d}/m_{s}\!=\!\gamma/2$),
...
$\Sigma m_{\nu}\!=\!0.0603$~eV (undressed-$m_3$ secondary
reading; the canonical reading using the carrier-defect-dressed
$m_{3}\!=\!0.05022$~eV is $\Sigma m_{\nu}\!=\!0.0593$~eV,
P4B~\cite{Paper4B} canonical closure) PRECISE in the
System-$\mathcal R$ refined closure
$m_{1}\!=\!\gamma^{2} m_{3}\!=\!m_{3}/100$ with the
oscillation-band $m_{3}\!=\!0.0511$~eV, the
...
resummation). The absolute eigenvalues
$m_{1}\!+\!m_{2}\!+\!m_{3}\!=\!0.0603$~eV are computed by the
companion paper's reproducer
\path|relational-uv-closure-repro/src/verify_neutrino_masses_m1_m2.py|;
the prediction matches the DESI DR2 BAO + DR1 full-shape
bound~\cite{DESI2025} $\Sigma m_{\nu}\!<\!0.0642$~eV (95\% C.L.,
flat-$\Lambda$CDM, three degenerate states) at $-6.1\%$ margin
and sits $+2.9\%$ above the oscillation lower limit
$\sqrt{\Delta m_{21}^{2}}\!+\!\sqrt{\Delta m_{31}^{2}}\!=\!0.0586$~eV,
a definite positive-$m_{1}$ excess. The tier is PRECISE rather
than EXACT: $\Sigma m_{\nu}$ is a derived sum of three
eigenvalues against a cosmological band, not a closed-form
$\mathcal R$-rational landing. The closure supersedes both the
legacy $m_{1}\!=\!m_{3}/(d{+}1)$ form ($\Sigma\!=\!0.0732$~eV,
DESI-falsified) and the earlier $m_{3}\!=\!0.0500$~eV scenario
($\Sigma\!=\!0.0591$~eV), which used a rounded $m_{3}$ and is
retained only as a superseded entry in the audit bundle
\pyown{data/closure_derivations/HY_neutrino_mass_sum_tension.json}{HY_neutrino_mass_sum_tension.json},
...
\pyown{src/verify_md_47_48_50_integration.py}{verify_md_47_48_50_integration}.
...
|V_{td}| &= \frac{2\,\alpha_{\rm EM}}{\sqrt{N_{\rm gen}}}\!\left(1\!+\!N_{\rm gen}\,\alpha_{\rm EM}\right)
                          = 8.60\!\times\!10^{-3} \nonumber \\
&\qquad\quad \text{(EXACT } 0.048\%\text{ vs PDG~2024 } 8.6\!\times\!10^{-3}\text{;}\nonumber\\
&\qquad\quad \text{tree }2\alpha_{\rm EM}/\sqrt{N_{\rm gen}}\!=\!8.42\!\times\!10^{-3}\text{ at }2.12\%\text{; QED dressing}\nonumber\\
&\qquad\quad (1\!+\!N_{\rm gen}\alpha_{\rm EM})\text{ from third-to-first generation }\alpha_{\rm EM}\text{-mediated correction})
\label{eq:Vtd_chirality} \\
|V_{cb}| &= \alpha_{\xi}/(2(2 d + N_{\rm gen})) \;=\; 9/220
                          \;=\; 0.04091 \nonumber \\
&\qquad\quad \text{(EXACT } 0.267\%\text{, vs PDG/HFLAV)}
\label{eq:Vcb_chirality} \\
m_{b}/m_{s} &= \frac{1}{\alpha_{\rm EM}\,N_{\rm gen}}\!\left(1\!-\!\frac{\gamma^{2}(d{+}N_{\rm gen})}{4}\right)
                              = 44.94 \nonumber \\
&\qquad\quad \text{(EXACT } 0.021\%\text{ vs PDG~2024 } 44.93\text{;}\nonumber\\
&\qquad\quad \text{tree }1/(\alpha_{\rm EM}\,N_{\rm gen})\!=\!45.74\text{ at }1.80\%\text{;}\nonumber\\
&\qquad\quad \text{universal }\gamma^{2}(d{+}N_{\rm gen})\text{-leakage at coefficient }1/d\text{,}\nonumber\\
&\qquad\quad \text{same form as atmospheric }m_{3}\text{ neutrino mass})
\label{eq:mb_ms_chirality} \\
\frac{\Delta m_{21}^{2}}{\Delta m_{31}^{2}}
            &= 4\,\alpha_{\rm EM} = 0.0292
\quad \text{(PRECISE } 0.86\%, \text{ NuFIT 6.1; bare-ratio reading;}\nonumber\\
&\qquad\quad \text{the dressed-} m_{2}^{2}/m_{3}^{2} \text{ form using
the universal $\gamma^{2}(d{+}N_{\rm gen})$-leakage}\nonumber\\
&\qquad\quad \text{recovers the ratio at the dressed-neutrino-mass level)}
\label{eq:Dm_ratio_chirality} \\
\sin^{2}\theta_{W}
            &= 1/4 - \varepsilon_{\rm sync}^{2}/N_{\rm gen}
            = 7/30 = 0.2333 \nonumber \\
            &\qquad\quad \text{(PRECISE } 0.91\%, \text{ vs PDG)}
...
(reproducer
\pyown{src/verify_alpha_em_pattern_consistency.py}{verify_alpha_em_pattern_consistency}; bundle
...
Three of the four original FACTOR2 entries
($|V_{cb}|$, $m_{t}/m_{b}$, $m_{c}/m_{u}$) are superseded by
sharper direct System-$\mathcal R$-rational closures catalogued in
the audit-completeness section above:
$|V_{cb}|\!=\!\axi/(2(2d\!+\!N_{\rm gen}))\!=\!9/220$ at EXACT
$0.267\%$;
$m_{t}/m_{b}\!=\!174.1/4.179\!=\!41.66$ at PRECISE
$0.85\%$ from $m_{t}$ spectral-Yukawa-eigenvalue and
$y_{b}\!=\!d!\,\gamma^{3}\!=\!3/125$;
$m_{c}/m_{u}\!=\!1270/2.159\!=\!588.2$ at EXACT $0.04\%$ from
$y_{c}\!=\!\alpha_{\rm EM}$ and the dressed
$y_{u}\!=\!(d{+}1)\gamma^{5}/4\cdot(1\!-\!(d{+}1)\gamma^{3})$.
Only the rare-process branching ratio
$\mathrm{BR}(B_{s}\!\to\!\mu^{+}\mu^{-})\!=\!\alpha_{\rm EM}^{4}$
at FACTOR2 $8.60\%$ remains without a direct System-$\mathcal R$
alternative. The CKM part of the physical prefactor does close
EXACT: with $V_{tb}\!=\!0.998$ from the SVD diagonal of the
canonical-regime $M_{f}$ and
$V_{ts}\!=\!\axi/(2(2d{+}N_{\rm gen}))\!=\!9/220$ (Wolfenstein-
degenerate with $V_{cb}$), the framework yields
$|V_{tb}V_{ts}^{*}|^{2}\!=\!1.6736\!\times\!10^{-3}$ vs
PDG~$1.679\!\times\!10^{-3}$ at EXACT $0.32\%$. The residual
$f_{B_{s}}^{2}$ factor is a hadronic decay constant, in the
same class as $\Lambda_{\rm QCD}, f_{\pi}, f_{K}, f_{B}$:
these are non-perturbative strong-interaction matrix elements
that the present framework treats as external lattice-QCD
inputs rather than direct System-$\mathcal R$-rational closures.
This is the single structural follow-up item from the
$\alpha_{\rm EM}$-cross-check audit. The lepton-mass and neutrino-mixing identities -- which
carry no quark RG-running between the electroweak scale and the
relevant low-energy observable -- close at PRECISE or better.
The combined audit is therefore consistent with the reading
that the $\alpha_{\rm EM}$ identification is a
\emph{cross-observable structural pattern} of the framework,
not a single-pair numerical coincidence.
...
PRECISE tier). The naive Thomson-limit $6.5\%$ gap is therefore
a renormalisation-scale mismatch, not a structural disagreement
of the framework. Reproducer in companion
paper~\cite{Paper6}
(\pyown{src/verify_alpha_W_self_consistency.py}{verify_alpha_W_self_consistency}; output
\pyown{outputs/verify_alpha_W_self_consistency.json}{verify_alpha_W_self_consistency.json}).
...
vs the Fermilab Muon~$g{-}2$ Collaboration's final 2025
measurement
$a_{\mu}^{\rm exp}\!=\!1.165\,920\,705(214)\!\times\!10^{-3}$~\cite{FermilabMuonG2_2025}
at $0.49\%$ residual (PRECISE tier). The remaining $\sim\!0.5\%$
is the sum of hadronic-vacuum-polarisation, electroweak, and
higher-order-QED contributions evaluated to PDG precision in
the data-driven literature~\cite{Davier2020} and, as of the
2025 Muon~$g{-}2$ Theory Initiative White
Paper~\cite{MuonG2WP2025}, to comparable precision in the
lattice-QCD HVP determination (Bazavov \textit{et al.}~2024
MILC complete short+intermediate+long-distance light-quark
connected contribution~\cite{Bazavov2024HVP}): the resulting
Standard-Model value
$a_{\mu}^{\rm SM}\!=\!1.165\,920\,33(62)\!\times\!10^{-3}$
($530$~ppb) agrees with the Fermilab~2025 final
measurement~\cite{FermilabMuonG2_2025} (127~ppb total uncertainty),
the historical $a_{\mu}$ anomaly being substantially resolved
at the current precision. The framework supplies the leading
$\alpha_{\rm EM}$ structural fixing; the universal Schwinger
relation then closes $a_{\mu}^{(1)}$ at the same precision as
$\alpha_{\rm EM}$ itself, with no additional free parameter.
...
\pyscript{emergent-gr-anisotropic-source-dm-de-repro}{src/verify_post_flip_dark_sector_theories.py}{verify_post_flip_dark_sector_theories},
\pyscript{emergent-gr-anisotropic-source-dm-de-repro}{src/verify_post_flip_theories_round3.py}{verify_post_flip_theories_round3},
\pyscript{emergent-gr-anisotropic-source-dm-de-repro}{src/verify_post_flip_theories_round7.py}{verify_post_flip_theories_round7},
\pyscript{emergent-gr-anisotropic-source-dm-de-repro}{src/verify_post_flip_theories_round8.py}{verify_post_flip_theories_round8}.
...
eigenvalue spectrum (regime-dependent). Under the
parameter-free Yukawa identities documented in the
audit-completeness section (cross-references in
parentheses) the bare-Yukawa hierarchy delivers
strict-EXACT closures on $9$ of $12$ fermion masses:
$m_{t}\!=\!174.1$~GeV bare-EXACT (Step-3/5 spectral
Yukawa eigenvalue);
$m_{b}\!=\!4.179$~GeV bare-EXACT $0.02\%$ via
$y_{b}\!=\!d!\,\gamma^{3}\!=\!3/125$;
$m_{c}\!=\!1.27$~GeV EXACT $0.06\%$ via
$y_{c}\!=\!\alpha_{\rm EM}$;
$m_{u}\!=\!2.159$~MeV strict-EXACT $0.04\%$ vs PDG~$2.16$ via
$y_{u}\!=\!(d{+}1)\gamma^{5}/4$ tree augmented by the
three-loop carrier-defect dressing $(1\!-\!(d{+}1)\gamma^{3})$;
$m_{\tau}$ EXACT (Eqs.~\eqref{eq:m_tau_sye_strict_exact}
spectral/$16/5$ on $5/10$ regimes, and
\eqref{eq:lepton_strict_exact} parameter-free
$D_{\Omega}^{3}$ at $0.49\%$);
$m_{\mu}\!=\!105.61$~MeV EXACT $0.045\%$ via $y_{\mu}$ tree
$+\,2$-loop compound $(1\!+\!11\gamma^{3})$, complemented
by parameter-free $D_{\Omega}^{2}$ at $0.43\%$
(Eq.~\eqref{eq:lepton_strict_exact});
$m_{e}\!=\!0.51129$~MeV EXACT $0.057\%$ via
$y_{e}\!=\!(29/10^{7})(1\!+\!5\gamma^{2}/4)$, complemented
by parameter-free $D_{\Omega}^{3}\alpha_{\xi}$ at $0.99\%$
(Eq.~\eqref{eq:m_e_strict_exact}). The neutrino sum
$\Sigma m_{\nu}\!=\!0.0593$~eV (canonical, P4B~\cite{Paper4B})
closes PRECISE via the $m_{1}\!=\!\gamma^{2}m_{3}$ refinement
applied to the carrier-defect-dressed
$m_{3}\!=\!0.05022$~eV; the older undressed-$m_3$ reading
gives $\Sigma m_{\nu}\!=\!0.0603$~eV (secondary), and the
NH-minimum reading
$\Sigma m_{\nu}\!\to\!m_{2}\!+\!m_{3}\!=\!0.0588$~eV is
strict-EXACT $0.05\%$ against the NuFIT~6.1 oscillation lower
bound (P4B Eqs.~p4b\_m3\_nu\_dressed, p4b\_m2\_nu\_dressed,
p4b\_Sigma\_mnu) under the parameter-free dressings
$(1\!-\!\gamma^{2}(d{+}N_{\rm gen})/4)$ for $m_{3}$ and
$(1\!+\!\gamma^{2}(d{+}N_{\rm gen}))$ for $m_{2}$, with both
sharing the same structural integer $d{+}N_{\rm gen}\!=\!7$ as
the CMB scalar tilt
$n_{s}\!=\!1\!-\!\gamma^{2}(d{+}N_{\rm gen})/2$.
$m_{d}, m_{s}$ close at PRECISE $\!\sim\!1.9\%$ via
$y_{d}, y_{s}$ tree forms; the ratio
$m_{d}/m_{s}\!=\!\gamma/2\!=\!\varepsilon_{\rm sync}^{2}\!=\!1/20$
is bare-EXACT (matter-core chirality-restriction identity).
Only $m_{1}$ (NH-minimum $m_{1}\!\to\!0$, or
$m_{1}\!=\!\gamma^{2}m_{3}$ in the see-saw refinement)
remains a structural endpoint rather than an independently
predicted absolute scale.
\item \emph{CKM matrix elements (two complementary routes).}
The framework delivers CKM closures through two independent
routes that together cover all nine elements at
FACTOR2-or-better. \emph{(i)~Bi-unitary SVD at canonical
regime $\mathcal P_{2}^{\prime}$} ($N\!=\!30$,
$\alpha_{\varphi}^{\rm derived}\!=\!9.919$) gives all
diagonal entries strict-EXACT
($V_{ud}\!=\!0.967$ at ratio $0.993$,
$V_{cs}\!=\!0.965$ at ratio $0.991$,
$V_{tb}\!=\!0.998$ at ratio $0.999$) and six off-diagonal
entries at FACTOR2 ratios $1.13$--$1.40$; the SVD pipeline
serves as a cross-check that the bi-unitary structure of the
framework matrix $M_{f}$ is consistent at the order of
magnitude. \emph{(ii)~Parameter-free structural lemma
identities} promote the off-diagonal entries to strict-EXACT:
$V_{us}\!=\!\alpha_{\xi}\,s_{\rm face}\!=\!9/40$
EXACT $0.004\%$,
$V_{cb}\!=\!\alpha_{\xi}/(2(2 d{+}N_{\rm gen}))\!=\!9/220$
EXACT $0.267\%$, $V_{ts}\!=\!9/220$ EXACT $0.22\%$
(Wolfenstein-degenerate with $V_{cb}$),
$V_{ub}\!=\!\gamma/N_{\rm gen}^{3}\!=\!1/270$ EXACT $0.37\%$,
$V_{td}\!=\!2\alpha_{\rm EM}/\sqrt{N_{\rm gen}}$
PRECISE $2.12\%$. Combined, the framework closes 7/9 CKM
elements at strict-EXACT (3 SVD diagonals + 4 lemma
off-diagonals), with $V_{td}$ at PRECISE and $V_{cd}$ at the
Wolfenstein-cabibbo-degenerate $|V_{us}|$ level. The two
routes are independent: the SVD pipeline operates on the
canonical-regime framework matrix, while the lemma identities
deliver direct framework-rational closures from the
$\{\gamma, \alpha_{\xi}, s_{\rm face}, N_{\rm gen}\}$ primitives.
...
\pyown{src/verify_sye_pipeline.py}{verify_sye_pipeline} (reads the bundled
five-step Yukawa-extraction output
\pyown{data/sye_y1_y5_canonical_regime.json}{sye_y1_y5_canonical_regime.json} and reports the
...
\item \emph{Charged-lepton absolute mass: factor-of-two closed.}
      ...
      (\pyown{src/verify_strict_exact_alpha_xi_omega.py}{verify_strict_exact_alpha_xi_omega}, output
      \pyown{outputs/verify_strict_exact_alpha_xi_omega.json}{verify_strict_exact_alpha_xi_omega.json})
      ...
      (\pyown{src/verify_strict_exact_roadmap.py}{verify_strict_exact_roadmap}) tests a
      ...
      the standard single-instanton action. The cascade-closure
      to $\sim\!10\%$ residual examined elsewhere would require a
      ...
      \pyown{src/verify_A_s_from_bounce.py}{verify_A_s_from_bounce} (cascade tests with
      retraction note),
      \pyown{src/verify_strict_exact_alpha_xi_omega.py}{verify_strict_exact_alpha_xi_omega}
...
\item \emph{Topology-tuple reading.} The closure depends on the
      ...
      and not extracted from a Lagrangian. $w$ and $r$ are
      explicit boolean flags on each registry entry, surfaced in
      ...
      transparent and falsifiable. An \emph{algorithmic} tuple
      compiler that derives $(n, g, s, w, r)$ directly from each
      observable's structural formula is bundled at
      \path|src/tuple_compiler/|: it takes per-observable YAML
      definitions from \texttt{data/observable\_definitions/} (with
      enforced target-value isolation: the extractor refuses to
      read any field that would expose the target value), and
      writes the extracted tuples plus provenance hashes to
      \pyown{outputs/extracted_topology_tuples.json}{extracted_topology_tuples.json} (50
      observables, $0$ extraction errors,
      \texttt{target\_isolation\_locked: true} on the current
      build). The compiler-extracted tuples match the
      classical-physics reading on the audited 50-observable set,
      removing the residual manual element from the
      observable\,$\to$\,tuple map.
...
\scriptsize
\input{tables/tab_frozen_tuples}
\normalsize
\end{table}
...
\bibitem{FermilabMuonG2_2025}
Muon $g{-}2$ Collaboration (D.~P.~Aguillard \textit{et al.}),
\emph{Final report of the muon $g{-}2$ experiment at Fermilab},
Physical Review Letters \textbf{134}, 245001 (2025);
arXiv:2506.03069.
$a_{\mu}^{\rm exp}\!=\!(1.165\,920\,705\!\pm\!114_{\rm stat}\!\pm\!91_{\rm syst})\!\times\!10^{-3}$
at $127$~ppb total precision, the final and most precise
single-experiment determination from the Fermilab E989 run.

\bibitem{MuonG2WP2025}
R.~Aliberti \textit{et al.} (Muon $g{-}2$ Theory Initiative),
\emph{The anomalous magnetic moment of the muon in the
Standard Model: an update},
arXiv:2505.21476 (2025).
Updated Standard-Model evaluation with lattice-QCD-based HVP:
$a_{\mu}^{\rm SM}\!=\!1.165\,920\,33(62)\!\times\!10^{-3}$
($530$~ppb), in agreement with the Fermilab final 2025
measurement to within the present joint precision.

\bibitem{Bazavov2024HVP}
A.~Bazavov \textit{et al.} (MILC Collaboration),
\emph{Hadronic vacuum polarization for the muon $g{-}2$ from
lattice QCD: Long-distance and full light-quark connected
contribution},
Physical Review Letters \textbf{135}, 011901 (2025);
arXiv:2412.18491.
Sub-percent precision lattice-QCD determination of the
light-quark-connected HVP contribution
$655.5(2.3)_{\rm stat}(3.9)_{\rm syst}[4.5]_{\rm total}\!\times\!10^{-10}$
combining short, intermediate, and long-distance windows; the
lattice-QCD HVP input that resolves the historical $a_{\mu}$
anomaly when combined with the Theory Initiative 2025
update~\cite{MuonG2WP2025} and the Fermilab final
measurement~\cite{FermilabMuonG2_2025}.
